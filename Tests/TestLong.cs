...
namespace Tests
{
    public class TestLong
    {
        private class Test
        {
            ...
            [Binary(Count = 1)]
            public long CountTest1 { get; set; }

            [Binary(Count = 2)]
            public long CountTest2 { get; set; }

            [Binary(Count = 3)]
            public long CountTest3 { get; set; }

            [Binary(Reverse = true)]
            public long ReverseTest { get; set; }

            [Binary(Reverse = true, Count = 1)]
            public long ReverseCountTest1 { get; set; }

            [Binary(Reverse = true, Count = 2)]
            public long ReverseCountTest2 { get; set; }

            [Binary(Reverse = true, Count = 3)]
            public long ReverseCountTest3 { get; set; }
        }

        [Theory]
        [InlineData(new byte[] // корректный
        {
            0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, //field
        ...
        public void Read(byte[] bytes, byte variant)
        {
            ...
            {
                case 1: //корректный
                    Assert.Equal(-9_223_372_036_854_775_807, test.Field);
        ...
        public void ReadReverse(byte[] bytes, byte variant)
        {
            ...
            switch (variant)
            {
                case 1: //корректный
                    ...
                    break;
                case 2: // недостаточно байтов 1
                    Assert.Equal(0x0100000000, test.Field);
                    Assert.Equal(0, test.Property);
                    Assert.Equal(0, test.CountTest1);
                    Assert.Equal(0, test.CountTest2);
                    Assert.Equal(0, test.CountTest3);
                ...
                case 3: // недостаточно байтов 2
                    Assert.Equal(100000, test.Field);
                    Assert.Equal(0x1000000000, test.Property);
                    Assert.Equal(0, test.CountTest1);
                    Assert.Equal(0, test.CountTest2);
                    Assert.Equal(0, test.CountTest3);
                    Assert.Equal(0, test.ReverseTest);
        ...
        public void Write()
        {
            Test test = new Test
            {
                Field = -9_223_372_036_854_775_807, Property = 100000, CountTest1 = 200, CountTest2 = 50000, CountTest3 = 16_777_215,
